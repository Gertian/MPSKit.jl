...
=#
"""
    QuasiparticleAnsatz_sweep <: Algorithm
...
The difference with `QuasiparticleAnsatz` is that this algorithm sweeps over the different Bs in a VUMPS like fashion rather then optimizing them all at once.
This should be more efficient for larger unit cells.

## Fields
- `alg::A = Defaults.eigsolver`: algorithm to use for the eigenvalue problem
- `tol::Real = Defaults.tol`: tolerance for energy convergence in the sweeps 
- `maxiter::Int = Defaults.maxiter`: maximum number of iterations in the sweeps

## Constructors
    
    QuasiparticleAnsatz_sweep()
    QuasiparticleAnsatz_sweep(; kwargs...)
    QuasiparticleAnsatz_sweep(alg, tol, maxiter)

Create a `QuasiparticleAnsatz_sweep` algorithm with the given algorithm, or by passing the 
keyword arguments to `Arnoldi`.
"""
struct QuasiparticleAnsatz_sweep{A} <: Algorithm
    alg::A
    tol::Real
    maxiter::Int
end
function QuasiparticleAnsatz_sweep(; kwargs...)
    alg = Defaults.alg_eigsolve(; dynamic_tols=false, kwargs...)
    return QuasiparticleAnsatz_sweep(alg,Defaults.tol,Defaults.maxiter)
end
...
function excitations(H, alg::QuasiparticleAnsatz_sweep, ϕ₀::InfiniteQP, lenvs, renvs;
                     num=1, solver=Defaults.linearsolver)
    qp_envs(ϕ) = environments(ϕ, H, lenvs, renvs; solver) #TODO: make a version of this that takes old environments as input.
    E_GS_renorm = effective_excitation_renormalization_energy(H, ϕ₀, lenvs, renvs)
    @show "jej I got here"
    H_eff = @closure(ϕ -> effective_excitation_hamiltonian(H, ϕ, qp_envs(ϕ), E_GS_renorm))
    @show "jej I also got here"
...
function excitations(H, alg::QuasiparticleAnsatz_sweep, ϕ₀::InfiniteQP, lenvs;
                     num=1, solver=Defaults.linearsolver)
...
function excitations(H, alg::QuasiparticleAnsatz_sweep, ϕ₀::InfiniteQP;
                     num=1, solver=Defaults.linearsolver)
...
"""
function excitations(H, alg::QuasiparticleAnsatz_sweep, momentum::Number, lmps::InfiniteMPS,
...
function excitations(H, alg::QuasiparticleAnsatz_sweep, momenta, lmps,
